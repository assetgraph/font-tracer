...
function fontTracer(
  document,
  { stylesheetsWithPredicates = [], getCssRulesByProperty, htmlAsset } = {}
) {
  ...
  const multipliedStyledTexts = _.flatten(
    styledTexts.map(({ props, ...rest }) =>
      expandPermutations(props)
        ...
        .filter(({ props }) =>
          CSS_PROPS_TO_TRACE_AND_TEXT.every(prop => {
        ...
        .map(({ props, ...rest }) => {
          ...
          const textTransform = props['text-transform'].value;
          if (textTransform !== 'none' && !props.text.isListIndicator) {
          ...
          const transformedStyledText = { text, ...rest, props: {} };
        ...
        .filter(function filterAndDeduplicate({ text, props }) {
          if (!text || props.display.toLowerCase() === 'none') {
            return false;
          }
