            ...
            if (/^['"]/.test(listStyleType)) {
              ...
              for (const counterStyle of counterStyles) {
                if (counterStyle.name === listStyleType) {
                  ...
                  const renderers = {};
                  for (const otherCounterStyle of counterStyles) {
                    if (otherCounterStyle !== counterStyle) {
                      renderers[counterStyle.name] = counteraction(
                        otherCounterStyle.props.system || 'decimal',
                        {
                          ...otherCounterStyle.props
                        }
                      );
                    }
                  }
                  ...
                  if (combinedPredicates) {
                    found = true;
                    const renderer = counteraction(counterStyle.props.system, {
                      ...counterStyle.props,
                      renderers
                    });
                    computedStyle.props.text.push({
